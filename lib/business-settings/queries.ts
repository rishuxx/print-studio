...
export async function getFullBusinessConfiguration(): Promise<FullBusinessConfiguration> {
  try {
    ...
    const business: BusinessSettingsRecord = {
      ...
      store_name: (rawBs?.business_name as string) || (rawBs?.store_name as string) || DEFAULT_BUSINESS_SETTINGS.store_name,
      legal_business_name: (rawBs?.legal_business_name as string) || DEFAULT_BUSINESS_SETTINGS.legal_business_name,
      display_name: (rawBs?.business_short_name as string) || (rawBs?.display_name as string) || DEFAULT_BUSINESS_SETTINGS.display_name,
      ...
      website_url: (rawBs?.canonical_site_url as string) || (rawBs?.website_url as string) || DEFAULT_BUSINESS_SETTINGS.website_url,
      currency_code: (rawBs?.currency_code as string) || "INR",
      currency_symbol: (rawBs?.currency_symbol as string) || "₹",
      timezone: (rawBs?.timezone as string) || "Asia/Kolkata",
      locale: (rawBs?.locale as string) || "en-IN",
